...
const SHEET_CSV_URL =
  ...
  'https://docs.google.com/spreadsheets/d/1CLjH67Sf9o2diBMUwiG9zmkhs47N90GmL4LxQ6TYsZk/gviz/tq?tqx=out:csv&sheet=Aus';
...
function readAuStandardStatus(meetsAustralianStandard, auStandardDetails, otherStandards) {
  const statedStatus = meetsAustralianStandard?.trim().toLowerCase();

  if (statedStatus === 'no' || statedStatus === 'false' || statedStatus === '0') {
    return 'does-not-meet';
  }

  if (inferAuStandard(meetsAustralianStandard, auStandardDetails, otherStandards)) {
    return 'meets';
  }
...
async function main() {
    ...
    .map((row) => {
      ...
      const auStdDetail =
        ...
        (readNullableString(entry.other_standards)?.toLowerCase().includes('as4989')
          ...
          : null);
      const auStdStatus = readAuStandardStatus(
      ...
      return {
        ...
        staticWeightKg: readNullableNumber(entry.static_weight_rating_kg),
        ...
        warrantyPartsYrs: readNullableNumber(entry.warranty_parts_years),
        auStdStatus,
        meetsAuStd: auStdStatus === 'meets',
        auStdDetail,
  ...
  const output = `// Generated by scripts/refresh-compare-data.mjs from the authoritative Bounce Arena spreadsheet's Aus tab.\n// Run \`npm run refresh:compare-data\` to update.\n\nexport type AuStandardStatus = 'meets' | 'does-not-meet' | 'not-confirmed';\n\nexport type Trampoline = {\n  brand: string; model: string; size: string; shape: string;\n  springless: boolean; springSystem: string | null; priceAud: number | null; priceBasis: string;\n  overallDiamCm: number | null; overallLenCm: number | null; overallWidCm: number | null;\n  matDiamCm: number | null; matLenCm: number | null; matWidCm: number | null; totalHeightCm: number | null;\n  maxWeightKg: number | null; combinedWeightKg: number | null; staticWeightKg: number | null; springCount: number | null;\n  warrantyFrameYrs: number | null; warrantyMatYrs: number | null; warrantyNetYrs: number | null; warrantyPartsYrs: number | null;\n  auStdStatus: AuStandardStatus; meetsAuStd: boolean; auStdDetail: string | null;\n  reviewSlug: string | null; baScore: number | null;\n  sourceUrl: string | null; goSlug: string | null;\n};\n\nexport const TRAMPOLINES: Trampoline[] = [\n${rows
    .map(
      (row) =>
        `  { brand: ${serialize(row.brand)}, model: ${serialize(row.model)}, size: ${serialize(row.size)}, shape: ${serialize(row.shape)}, springless: ${row.springless}, springSystem: ${serialize(row.springSystem)}, priceAud: ${serialize(row.priceAud)}, priceBasis: ${serialize(row.priceBasis)}, overallDiamCm: ${serialize(row.overallDiamCm)}, overallLenCm: ${serialize(row.overallLenCm)}, overallWidCm: ${serialize(row.overallWidCm)}, matDiamCm: ${serialize(row.matDiamCm)}, matLenCm: ${serialize(row.matLenCm)}, matWidCm: ${serialize(row.matWidCm)}, totalHeightCm: ${serialize(row.totalHeightCm)}, maxWeightKg: ${serialize(row.maxWeightKg)}, combinedWeightKg: ${serialize(row.combinedWeightKg)}, staticWeightKg: ${serialize(row.staticWeightKg)}, springCount: ${serialize(row.springCount)}, warrantyFrameYrs: ${serialize(row.warrantyFrameYrs)}, warrantyMatYrs: ${serialize(row.warrantyMatYrs)}, warrantyNetYrs: ${serialize(row.warrantyNetYrs)}, warrantyPartsYrs: ${serialize(row.warrantyPartsYrs)}, auStdStatus: ${serialize(row.auStdStatus)}, meetsAuStd: ${row.meetsAuStd}, auStdDetail: ${serialize(row.auStdDetail)}, reviewSlug: ${serialize(row.reviewSlug)}, baScore: ${serialize(row.baScore)}, sourceUrl: ${serialize(row.sourceUrl)}, goSlug: ${serialize(row.goSlug)} },`,
    )
    .join('\n')}\n];\n`;
