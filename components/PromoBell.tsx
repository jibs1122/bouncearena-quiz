...
function ShopLink({
  ...
  href: string;
  location: string;
  /** Plain text link for the phone card, where a button per row costs too much height. */
  inline?: boolean;
}) {
  const shared = {
    href,
  ...
  if (inline) {
    return (
      <a
        {...shared}
        className="shrink-0 rounded py-1 text-xs font-bold text-[#38b1ab] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#38b1ab]"
      >
        Shop →
      </a>
    );
  ...
  return (
    <a
      {...shared}
      className="inline-flex min-h-9 w-full items-center justify-center rounded-lg bg-[#38b1ab] px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-[#2e9a94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#38b1ab] focus-visible:ring-offset-2"
    >
      Shop {brand}
    </a>
  );
    ...
    <>
      ...
      {!mobileClosed && (
        <aside aria-label="Promo codes" className="fixed bottom-3 left-0 right-0 z-30 px-3 lg:hidden">
          ...
          <div className="max-w-full overflow-hidden rounded-2xl border border-black/10 bg-white/96 px-3.5 pb-1.5 pt-1 shadow-[0_14px_34px_-22px_rgba(0,0,0,0.45)] backdrop-blur">
            <div className="flex items-center justify-between gap-3">
              <span className="text-[13px] font-bold leading-4 text-[#278984]">Current promo codes</span>
              <button
                type="button"
                aria-label="Close promo codes"
                onClick={() => setMobileClosed(true)}
                className="-mr-1.5 flex h-6 w-6 items-center justify-center rounded-full text-black/50 transition-colors hover:bg-black/5 hover:text-black/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#38b1ab]"
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                  <path d="M6 6l12 12" />
                  <path d="M18 6L6 18" />
            ...
            <div>
              {PROMOS.map((promo) => (
                <div key={promo.brand} className="flex min-h-6 min-w-0 items-center justify-between gap-3">
            ...
            <p className="mt-0.5 text-[10px] leading-3 text-black/50">
              Affiliate links. We may earn a commission.
            </p>
          </div>
        </aside>
